...
rman_script(){
cat <<SORM > ${ORACLE_SCRIPT_PATH}/rman_oracle.sh
#!/bin/bash
...
ORMF
SORM
echo "10 0 * * * sh ${ORACLE_SCRIPT_PATH}/rman_oracle.sh >> /tmp/rman_oracle.log 2>&1" >> /var/spool/cron/oracle
chmod 600 /var/spool/cron/oracle
chown oracle:oinstall /var/spool/cron/oracle ${ORACLE_SCRIPT_PATH}/rman_oracle.sh
}
...
每天0点10分执行备份任务，备份日志 "/tmp/rman_oracle.log"
CONFIGURE CONTROLFILE AUTOBACKUP ON;
CONFIGURE RETENTION POLICY TO REDUNDANCY 3;
CONFIGURE DEVICE TYPE DISK PARALLELISM 2 BACKUP TYPE TO COMPRESSED BACKUPSET;
