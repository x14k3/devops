...
```sql
-- 备份数据库
mysqldump -uroot -p databaseName > /opt/test.sql
GRANT PROCESS ON mysql.proc to 'jy2web'; 
flush privileges;
quit; 
...
这里的我的数据文件均存放在/data/mysql\_data目录下

```bash
cd /data/mysql_data/  ; rm -rf ./*
```
...
```bash
innobackupex --defaults-file=/etc/my.cnf --apply-log /opt/backup/full/
```

copy-back恢复数据

```bash
innobackupex --defaults-file=/etc/my.cnf --copy-back /opt/backup/full/
```
