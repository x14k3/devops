...
- `limit_conn` 限制某个`ip`的`tcp`连接数目或者限制某个`server`（网站）整体的连接数目
- `limit_rate` 限制每个请求的数据大小
- `limit_req` 限制某个`ip`的请求次数
...
如果你的站点是`http1.1`，连接数目 \= 请求数目/ \<浏览器允许并发的数目\>
如果你的站点是`http2.0` 连接数目 \= 1

`http1.1` 默认开启`keep-alive` 特性，支持`tcp`持久连接，但是由于浏览器限制了并发数目，所以连接太多，仍然会进行再次的`tcp`连接。
而`http2`引入了多路复用和二进制帧分层特性，允许所有的请求来自同一个`tcp`。

需要注意的是`http2.0`只适用于`https`的站点，并且需要在服务器端进行配置。

`http`站点仍然使用`http1.1`协议。

下面是`nginx`配置配置文件中配置`http2`的一个例子：
...
```nginx
limit_req_zone $binary_remote_addr zone=zonename:10m rate=20r/s;
```
...
- ​`zone`​: 后面的`zonename`​是域名称，可以随便取一个，下面对应就可以了，冒号后面的10m 表示了这个域的大小，即该模块需要开辟一个内存空间是缓存nginx的请求记录中的`$binary_remote_addr`，以便匹配出请求速率是否超过了指定的`rate`。
- ​`rate`​: 这个是1秒中允许的请求数目，这个地方很容易被误解导致配置错误。比如`rate`设置为`20r/s`，表示每秒20个请求，平均50ms只允许有一个请求，因为nginx是毫秒级的速率控制，这个地方的rate实际上是毫秒级的匀速控制。但实际上我们的站点的请求都是突发流量，即在短时间内很多个请求并发的。所以需要使用到burst来承接突发流量。
...
### ngx\_http\_limit\_conn\_module 限制连接数
...
### ngx\_http\_core\_module 限制下载速度
