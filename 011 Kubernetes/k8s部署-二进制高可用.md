...
|主机名称|IP地址|说明|软件|
| ----------| --------------| ------------| ----------------------------------------------------------------------------------------------------------------------------------------|
|Master01|192.168.8.61|master节点|kube-apiserver、kube-controller-manager、kube-scheduler、etcd、kubelet、kube-proxy、nfs-client、haproxy、keepalived、nginx、containerd|
|Master02|192.168.8.62|master节点|kube-apiserver、kube-controller-manager、kube-scheduler、etcd、kubelet、kube-proxy、nfs-client、haproxy、keepalived、nginx、containerd|
|Master03|192.168.8.63|master节点|kube-apiserver、kube-controller-manager、kube-scheduler、etcd、kubelet、kube-proxy、nfs-client、haproxy、keepalived、nginx、containerd|
|Node01|192.168.8.64|node节点|kubelet、kube-proxy、nfs-client、nginx、containerd|
|Node02|192.168.8.65|node节点|kubelet、kube-proxy、nfs-client、nginx、containerd|
||192.168.8.66|VIP||
...
在《5.高可用配置》选择使用那种高可用方案 若使用 haproxy、keepalived 那么为 `--server=https://192.168.8.66:8443`​ 若使用 nginx方案，那么为 `--server=https://127.0.0.1:8443`​
...
_*注意* 5.1.1 和5.1.2 二选一即可_*

选择使用那种高可用方案

在《3.2.生成k8s相关证书》

若使用 nginx方案，那么为 `--server=https://127.0.0.1:8443`​ 若使用 haproxy、keepalived 那么为 `--server=https://192.168.8.66:8443`​
...
# 查看集群状态，没问题的话继续后续操作
[root@k8s-master01 metrics-server]# kubectl get cs
...
查看calico支持的kubernetes版本：[https://projectcalico.docs.tigera.io/archive/v3.24/getting-started/kubernetes/requirements](https://projectcalico.docs.tigera.io/archive/v3.24/getting-started/kubernetes/requirements)

在线下载calico.yaml配置文件地址: [https://docs.projectcalico.org/manifests/calico.yaml](https://docs.projectcalico.org/manifests/calico.yaml)

calico github地址：[https://github.com/projectcalico/calico/releases](https://github.com/projectcalico/calico/releases)

### 9.1.1更改calico网段

v3.26.1: [calico.yaml](assets/calico-20230815122720-9b3f10k.yaml)
...
```shell
[root@k8s-master01 metrics-server]# kubectl  get pod -A
NAMESPACE     NAME                                       READY   STATUS    RESTARTS      AGE
kube-system   calico-kube-controllers-74cfc9ffcc-97ppm   1/1     Running   0             121m
kube-system   calico-node-bwvh4                          1/1     Running   0             121m
kube-system   calico-node-f2brt                          1/1     Running   0             121m
kube-system   calico-node-trl89                          1/1     Running   0             121m
kube-system   calico-node-vpx7c                          1/1     Running   0             121m
kube-system   calico-node-xnbxw                          1/1     Running   1 (26m ago)   121m
kube-system   coredns-78cdc77856-779vs                   1/1     Running   0             3m
kube-system   metrics-server-6bbcb9f574-8bdx5            1/1     Running   0             16s

[root@k8s-master01 metrics-server]# kubectl  top node
NAME           CPU(cores)   CPU%   MEMORY(bytes)   MEMORY%   
k8s-master01   121m         6%     1075Mi          28%   
k8s-master02   165m         8%     1536Mi          40%   
k8s-master03   163m         8%     1491Mi          39%   
k8s-node01     100m         5%     755Mi           19%   
k8s-node02     75m          3%     714Mi           18%     
```
...
NAME                                READY   STATUS    RESTARTS   AGE   IP               NODE           NOMINATED NODE   READINESS GATES
busybox                             1/1     Running   0          39m   172.16.195.1     k8s-master03   <none>           <none>
nginx-deployment-6ff84ff895-kkbp9   1/1     Running   0          70s   172.16.32.130    k8s-master01   <none>           <none>
nginx-deployment-6ff84ff895-lwkxd   1/1     Running   0          70s   172.16.122.129   k8s-master02   <none>           <none>
nginx-deployment-6ff84ff895-rrpc6   1/1     Running   0          70s   172.16.58.193    k8s-node02     <none>           <none>
...
[dashboard.yaml](assets/dashboard-20230815152021-6wc3fm0.yaml)

[dashboard-user.yaml](assets/dashboard-user-20230815152024-hxoqamo.yaml)
...
NAME                   TYPE       CLUSTER-IP       EXTERNAL-IP   PORT(S)         AGE
kubernetes-dashboard   NodePort   10.102.106.45   <none>        443:31215/TCP   2m28s
```
...
[https://192.168.2.11:31215](https://192.168.2.11:31215)
