...
```sh
SH$ git pull <远程主机名> <远程分支名>:<本地分支名>           #取回<远程主机名>的<远程分支名>，并与<本地分支名>合并
...
```sh
SH$ git rm test.txt                #将 text.txt 从磁盘删除，并移除 索引
...
```sh
SH$ git reset --hard HEAD~1       #撤销最近一次commit, 数字可变。 在这之后的 commit 全部舍弃
...
```sh
SH$ git add testt.txt                  #将 text.txt 添加到索引
...
```sh
SH$ git checkout branchname            #切换新分支 head 版本
...
```sh
SH$ git checkout deepzz                #切换到deepzz分支
...
```sh
SH$ git checkout deepzz    #切换到deppzz分支
...
```sh
SH$ git config --global user.name "your_name"          #配置全局的用户名和邮箱
...
```sh
SH$ git clone B_REPOSITORY_URL                        #先把B clone到本地
