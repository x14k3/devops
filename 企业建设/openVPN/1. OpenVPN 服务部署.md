...
第一条命令允许客户端使用服务器作为Internet代理，它将客户端流量（网络10.8.0.x）转发到服务器上的eth0网络接口上。eth0接口通常是广域网接口。  
...
# 手动启动
#openvpn --daemon --cd /etc/openvpn --config server.ovpn --log-append /var/log/openvpn.log
...
```bash
root@vps:/data# ll /lib/systemd/system/ | grep openvpn
...
主要区别总结
1. `openvpn.service` - 主服务文件
	这是 OpenVPN的传统服务文件
	默认已废弃或很少使用
	主要供兼容性考虑

2. `openvpn@.service` - **模板服务（推荐使用）**
	最重要的服务文件 - 最常用！
	模板格式：``openvpn@配置文件名`
	可以同时启动多个 OpenVPN连接
	配置文件存放在 `/etc/openvpn/client/` 或 `/etc/openvpn/server/`
	`sudo systemctl start openvpn@myvpn`

3. `openvpn-client@.service`- **客户端模板**
	专门用于 OpenVPN客户端连接
	板格式：`openvpn-client@配置文件名`
	配置文件通常在` /etc/openvpn/client/`
	
4. `openvpn-server@.service` - **服务器模板**
	专门用于 OpenVPN服务器实例
	模板格式：`openvpn-server@配置文件名`
	配置文件通常在 /etc/openvpn/server/
