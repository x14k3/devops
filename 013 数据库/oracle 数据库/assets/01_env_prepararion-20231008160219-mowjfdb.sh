...
export ORACLE_RECOVER_PATH=\${ORACLE_INSTALL_DIR}/orarecover
...
EOF
if [ $? -eq 0 ];then
	echo -e "\e[1;36m  ${SYSCTL}  Modification completed \e[0m"
fi
sysctl -p > /dev/null
}
...
EOF
if [ $? -eq 0 ];then
	echo -e "\e[1;36m  ${LIMITS}  Modification completed \e[0m"
fi
}
...
EOF
if [ $? -eq 0 ];then
	echo -e "\e[1;36m  ${PAM}  Modification completed \e[0m"
fi
}
...
EOF
source ${ETC_PROFILE}
echo -e "\e[1;36m  ${ETC_PROFILE}  Modification completed \e[0m"
}
...
echo -e "\e[1;36m  ${ORACLE_PROFILE}  Modification completed  \e[0m"
