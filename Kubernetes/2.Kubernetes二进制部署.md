...
| 主机名称 | IP地址    | 说明       | 软件                                                         |
| -------- | --------- | ---------- | ------------------------------------------------------------ |
| Master01 | 192.168.1.61 | master节点 | kube-apiserver、kube-controller-manager、kube-scheduler、etcd、<br />kubelet、kube-proxy、haproxy、keepalived |
| Master02 | 192.168.1.62 | master节点 | kube-apiserver、kube-controller-manager、kube-scheduler、etcd、<br />kubelet、kube-proxy、haproxy、keepalived |
| Master03 | 192.168.1.63 | master节点 | kube-apiserver、kube-controller-manager、kube-scheduler、etcd、<br />kubelet、kube-proxy、haproxy、keepalived |
| Node01   | 192.168.1.64 | node节点   | kubelet、kube-proxy                        |
| Node02   | 192.168.1.65 | node节点   | kubelet、kube-proxy                        |
|          | 192.168.1.66 | VIP        |                                            |

| 软件   | 组件说明                        | 版本         |
| :----------|------------------ | :----------- |
| kernel      | 内核建议升级到5版本       | Linux 5.4.231-1.el7.elrepo.x86_64|
| CentOS 7    |     | v8 或者 v7   |
| kube-apiserver、kube-controller-manager、kube-scheduler、kubelet、kube-proxy || v1.25.0      |
| etcd    | _Kubernetes_ 组件都是_无状态_的，所有的群集状态都储存在_etcd_ 集群中 | v3.5.4       |
| containerd  |容器运行时     | v1.6.8       |
| docker                                                       | v20.10.17    |
| cfssl                                                        | v1.6.2       |
| cni                                                          | v1.1.1       |
| crictl                                                       | v1.6.8      |
...
安装包下载：https://github.com/cby-chen/Kubernetes/releases/download/v1.25.0/kubernetes-v1.25.0.tar
...
- cfssl二进制包
	...
	`wget -O cfssl_1.6.2_linux_amd64.tar.gz https://github.com/cloudflare/cfssl/releases/download/v1.6.2/cfssl_1.6.2_linux_amd64`
	`wget -O cfssljson_1.6.2_linux_amd64.tar.gz https://github.com/cloudflare/cfssl/releases/download/v1.6.2/cfssljson_1.6.2_linux_amd64`
	`wget -O cfssl-certinfo_1.6.2_linux_amd64.tar.gz https://github.com/cloudflare/cfssl/releases/download/v1.6.2/cfssl-certinfo_1.6.2_linux_amd64`
...
```shell
# 方式一,禁用 NetworkManager，使用network
# systemctl disable --now NetworkManager
# systemctl start network && systemctl enable network

# 方式二，使用NetworkManager
...
# grubby --set-default /boot/vmlinuz-5.4.231-1.el7.elrepo.x86_64
...
## 2.1.所有k8s节点安装Containerd作为Runtime（推荐）
从kubernetes 1.24开始，docker不能再作为kubernetes的容器运行时了，推荐使用Containerd；
...
cfssl gencert \
   ...
   -hostname=127.0.0.1,k8s-master01,k8s-master02,k8s-master03 \
   -profile=kubernetes \
...
## 5.3Master01配置keepalived master节点
...
## 5.4Master02配置keepalived backup节点
...
## 5.5Master03配置keepalived backup节点
