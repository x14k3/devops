...
|字符|八进制表示法|权限|对文件的含义|对目录的含义|
| ------| --------------| ------| ----------------------------------------------| ------------------------------|
|r|4|读|意味着我们可以查看阅读|可以列出目录中的文件列表|
|w|2|写|意味着，对文件可以修改或删除|可以在该目录中创建、删除文件|
|x|1|执行|如果是文件就可以运行，比如二进制文件或脚本。|可以使用cd命令进入该目录|

那三种访问级别都有哪些呢？每个文件都有三组不同的读、写和执行权限，分别适用于三种访问级别，其中每组中的三个栏位分别使用读取权限（r）、写入权限（w）、执行权限（x）或没有相应的权限（-）来表示，共9位来表示。

![1571370358704.png](assets/net-img-1602399589987-20230907111732-o1q6oev.png)
...
![1571375694210.png](assets/net-img-1602399812604-20230907111732-isyy9bo.png)
